...
def fourier():
    ...
    for j in range (len(x)):
        for i in range(1,n+1):
            y[j] +=  (np.cos((np.pi*i*x[j])/.800) * T_an[i-1]) + (np.sin((np.pi*i*x[j])/.800) * T_bn[i-1])
            y[j] +=  (np.cos((np.pi*i*x[j])/.800) * R_an[i-1])
            y[j] +=  (np.cos((np.pi*i*x[j])/.800) * S_an[i-1])
            y[j] +=  (np.cos((np.pi*i*x[j])/.800) * P_an[i-1]) + (np.sin((np.pi*i*x[j])/.800) * P_bn[i-1])
        y[j] += (R_a0/2)
    return x, y
