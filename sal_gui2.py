...
def set_new_window(root):
    ...
    new_hr_width_side = new_ecg_width_size * .20  # heart rate width for frame

    # Create the main ECG frame
    new_ecg_frame = ttk.Frame(root)
    new_ecg_frame.pack(padx=10, pady=10, fill='both', expand=True)

    # Configure columns in the frame
    new_ecg_frame.columnconfigure(0, weight=1)
    new_ecg_frame.columnconfigure(1, weight=1)
    new_ecg_frame.columnconfigure(2, weight=1)

    #### LEAD II FRAME AND LABEL 
    new_leadII_label = ttk.Label(new_ecg_frame, text="LEAD II", foreground="#84f91c")
    new_leadII_label.grid(row=0, column=0, sticky="w")
    new_btm2_frame = ttk.Frame(new_ecg_frame, height=new_frame_size_height, width=new_frame_size_width)
    new_btm2_frame.grid(row=1, column=0)

    # LEAD V1 FRAME AND LABEL 
    new_leadV1_label = ttk.Label(new_ecg_frame, text="V1", foreground="#84f91c")
    new_leadV1_label.grid(row=2, column=0, sticky="w")
    new_btm3_frame = ttk.Frame(new_ecg_frame, height=new_frame_size_height, width=new_frame_size_width)
    new_btm3_frame.grid(row=3, column=0)
    new_V1_signal_frame = tk.Frame(new_btm3_frame, height=new_frame_size_height, width=new_frame_size_width)
    new_V1_signal_frame.grid(row=0, column=0, sticky='w')

    # Bottom frame for ECG (Pulse, awRR, TPERI)
    new_btm4_frame = ttk.Frame(new_ecg_frame, height=new_bot_frame_size_h, width=new_bot_frame_size_w)
    new_btm4_frame.grid(row=4, column=0)

    # ECG Live Signal with HR
    new_ecg_signal_frame = tk.Frame(new_btm2_frame, height=new_frame_size_height, width=new_frame_size_width)
    new_ecg_signal_frame.grid(row=0, column=0)

    new_HR_frame = tk.Frame(new_ecg_frame, width=new_hr_width_side, height=100)
    new_HR_frame.grid(row=1, column=1)
    new_HR_canvas = tk.Canvas(new_HR_frame, highlightthickness=0, bg="#84f91c", width=100, height=100)
    new_HR_canvas.grid(row=0, column=2)
    new_Heart_rate = new_HR_canvas.create_text(10, 10, text="HR", font=('Arial', 15), fill="red")

    new_d_canvas = tk.Canvas(new_HR_frame, bg="#84f91c", bd=0, highlightcolor="#84f91c", highlightthickness=0, width=18, height=20)
    new_d_canvas.grid(row=0, column=0)
    new_dot = new_d_canvas.create_oval(2, 2, 18, 18, fill='red', outline="")
    new_rate = new_HR_canvas.create_text(50, 50, text="--", font=('Arial', 40), fill="#99f20f")

    # Bottom frame for Pulse, awRR, TPERI
    new_pulse_frame = tk.Frame(new_btm4_frame, width=new_bot_frame_size_w, height=new_bot_frame_size_h, padx=2)
    new_pulse_frame.grid(row=0, column=1)
    new_pulse_canvas = tk.Canvas(new_pulse_frame, highlightthickness=0, bd=0, bg="black", height=new_bot_frame_size_h, width=100)
    new_pulse_canvas.grid(row=0, column=0)
    new_pulse = new_pulse_canvas.create_text(20, 20, text="Pulse", font=('Arial', 15), fill="#0fbef2")
    new_pulse_result = new_pulse_canvas.create_text(70, 70, text="--", font=('Arial', 50), fill="#0fbef2")

    new_awRR_frame = tk.Frame(new_btm4_frame, width=new_bot_frame_size_w, height=new_bot_frame_size_h, padx=2)
    new_awRR_frame.grid(row=0, column=2)
    new_awRR_canvas = tk.Canvas(new_awRR_frame, highlightthickness=0, bd=0, bg="black", width=100, height=new_bot_frame_size_h)
    new_awRR_canvas.grid(row=0, column=0)
    new_awRR = new_awRR_canvas.create_text(20, 20, text="awRR", font=('Arial', 15), fill="white")
    new_awRR_result = new_awRR_canvas.create_text(70, 70, text="--", font=('Arial', 50), fill="white")

    new_Tperi_frame = tk.Frame(new_btm4_frame, width=new_bot_frame_size_w, height=new_bot_frame_size_h, padx=2)
    new_Tperi_frame.grid(row=0, column=3)
    new_Tperi_canvas = tk.Canvas(new_Tperi_frame, highlightthickness=0, bd=0, bg="black", width=100, height=new_bot_frame_size_h)
    new_Tperi_canvas.grid(row=0, column=0)
    new_Tperi = new_Tperi_canvas.create_text(20, 20, text="Tperi", font=('Arial', 15), fill="#84f91c")
    new_Tperi_result = new_Tperi_canvas.create_text(70, 70, text="--", font=('Arial', 50), fill="#84f91c")
    
    widgets = {
    "leadII_label": new_leadII_label,
    "btm2_frame": new_btm2_frame,
    "btm3_frame": new_btm3_frame,
    "V1_signal_frame": new_V1_signal_frame,
    "ecg_signal_frame": new_ecg_signal_frame,
...
def ecg_start():
    if (clicked.get() == "Heart Rhytms" and len(y) == 0):
    ...
    # turn off start
    if (clicked.get() != "Heart Rhytms" or len(y) != 0):
...
def animation(ecg_num,widgets, copy):
    ...
    # Embed fig in canvas
    canvas = FigureCanvasTkAgg(fig1, widgets['ecg_signal_frame'])
    canvas2 = FigureCanvasTkAgg(fig2, widgets['V1_signal_frame'])
    # draw ecg signal
    canvas.draw()
...
# button clicked
def on_click(event):
    x, y = window.winfo_screenwidth(), window.winfo_screenheight()#pgui.position()
...
btn_update = ttk.Button(left_frame, text="Update", style="S.TButton", command=on_click_update)
btn_x_update = int((left_width/3))
...
placeholder = ttk.LabelFrame(bottom_frame, text= "", width=save_width_size, height=save_height_size)
...
widgets = {
    ...
    "btm2_frame": btm2_frame,
    "btm3_frame": btm3_frame,
    "V1_signal_frame": V1_signal_frame,
    "ecg_signal_frame": ecg_signal_frame,
    "HR_frame": HR_frame,
    "HR_canvas": HR_canvas,
