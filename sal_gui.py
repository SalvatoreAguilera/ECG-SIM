...
def animation(window,ecg_num):
    ...
    if(clicked.get() == 'Normal Rhythm'):
        ...
        t = np.linspace(-2,2,1200)
        y = normal.vfib()
    elif (clicked.get() == 'Atrial Tachycardia'):
        t = np.linspace(-2,2,1200)
        y = normal.atrial_tachy()
    elif (clicked.get() == 'VT'):
        t = np.linspace(-2,2,1200)
        y = normal.VT()
    elif (clicked.get() == 'Atrial Flutter'):
        t = np.linspace(-2,2,1200)
        y = normal.aflutt()
    elif (clicked.get() == 'Multifocal Atrial Tachycardia'):
        t = np.linspace(-2,2,1200)
        y = normal.multi_tachy()
    else:
        t = np.linspace(-2,2,1200)
        y = normal.update_ecg()
