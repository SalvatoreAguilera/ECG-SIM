...
def animation(window,ecg_num):
    ...
    plt.xticks(np.arange(min(t),max(t),step=0.5))
    plt.yticks(np.arange(min(y),max(y),step=0.25))
    ...
    # call animation
    animate = FuncAnimation(fig= fig, func= update,frames=len(t),interval=25, repeat=TRUE)
...
bottom_frame.grid(row=2,column=0, columnspan=3, padx =10, pady=10, sticky='nesw')
